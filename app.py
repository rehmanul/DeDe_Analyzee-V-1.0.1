...
import tempfile
import os
import numpy as np  # Added for np.mean in display_furniture_catalog and display_statistics
...
try:
    ...
    class FurnitureCatalogManager:
        ...
        def recommend_furniture_for_space(self, space_type, space_area, budget,
...
if 'direct_parse_results' not in st.session_state:  # Added for direct parser
    st.session_state.direct_parse_results = None
if 'current_filename' not in st.session_state:  # Added for direct parser
...
def display_main_interface(components):
    ...
    if st.session_state.advanced_mode:
        ...
        tabs = st.tabs([
            ...
            "Database & Projects",
            "CAD Export",
            "Direct DWG Parser",  # Added new tab
            "Settings"
        ])
        ...
        with tabs[7]:  # New tab for direct parser
            display_direct_dwg_parser_interface()
        with tabs[8]:
        ...
        with tabs[2]:
            display_statistics()
        with tabs[3]:  # New tab for direct parser
            display_direct_dwg_parser_interface()
        with tabs[4]:
...
def generate_comprehensive_report(components):
    ...
    try:
        with st.spinner("Generating comprehensive report..."):
            ...
            # Generate CSV data
            csv_data = export_manager.export_to_csv(
                st.session_state.analysis_results)
...
def load_multiple_dwg_files(uploaded_files):
    ...
    try:
        with st.spinner("Loading multiple DWG files..."):
            all_zones = []
            floor_plans = []
            parser = DWGParser()  # Initialize parser once
            ...
            for i, file in enumerate(uploaded_files):
                ...
                with tempfile.NamedTemporaryFile(
                        delete=False,
                        suffix=Path(file.name).suffix) as tmp_file:
...
def run_advanced_analysis(components):
    ...
    try:
        with st.spinner("Running advanced AI analysis..."):
            ...
            optimization_results = ai_analyzer.optimize_furniture_placement(
                st.session_state.zones, params)
            ...
            if 'current_project_id' in st.session_state:
                analysis_id = db_manager.save_analysis_results(
                    st.session_state.current_project_id, 'advanced', params,
                    results)
...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_file = st.file_uploader(
            ...
            help=
            "Upload architectural plan files for analysis (Max 200MB per file)"
        )
            ...
            try:
                if isinstance(uploaded_file, list):
                    ...
                    total_size = sum(f.size
                                     for f in uploaded_file) / (1024 * 1024)
    ...
    # Main content area
    try:
        if not st.session_state.dwg_loaded and st.session_state.direct_parse_results is None:  # Check for direct parse results too
    ...
    if 'rooms' in results:
        ...
        with col2:
            ...
            if 'total_boxes' in results:
                box_area = results.get('total_boxes', 0) * results.get(
                    'parameters', {}).get(
                        'box_size', [2.0, 1.5])[0] * results.get(
    ...
    try:
        with st.spinner("Loading and parsing DWG file..."):
            ...
            try:
                ...
                if not zones:
                    st.warning(
                        "No valid zones found in the file. Please check if the file contains closed polygons or room boundaries."
                    )
                    return
...
def display_analysis_results():
    ...
    for zone_name, room_info in results.get('rooms', {}).items():
        ...
        room_data.append({
            ...
            'Dimensions':
            f"{room_info.get('dimensions', [0, 0])[0]:.1f} × {room_info.get('dimensions', [0, 0])[1]:.1f}",
...
def display_statistics():
    ...
    with col2:
        ...
        if placement_counts:
            ...
            fig_bar = px.bar(df, x='Zone', y='Count', title="Boxes per Zone")
        else:
            fig_bar = px.Figure()
...
# --- Start of Merged direct_dwg_parser.py functions ---

from shapely.geometry import Polygon


def parse_dwg_file_direct(file_path):
    """Parse DWG file and extract room data using DWGParser"""
    try:
        parser = DWGParser()
        zones = parser.parse_file_from_path(file_path)

        results = {
            'layers':
            list(set([zone.get('layer', 'Unknown') for zone in zones])),
            'entities': {
                'ROOMS': len(zones)
            },
            'rooms': []
        ...
        for zone in zones:
            if 'points' in zone and len(zone['points']) >= 3:
                try:
                    # Ensure points are tuples for shapely Polygon
                    shapely_points = [tuple(p) for p in zone['points']]
                    polygon = Polygon(shapely_points)
                    area = zone.get('area', polygon.area)
                    ...
                    results['rooms'].append({
                        ...
                        area,
                        'layer':
                        zone.get('layer', 'Unknown'),
                        'perimeter':
                        perimeter,
                        'type':
                        zone.get('type', 'Unknown'),
                        'source':
                        zone.get('source', 'parsed')
                    })
                except Exception as poly_e:
                    # Fallback if shapely fails (e.g., non-simple polygon)
                    st.warning(
                        f"Shapely failed for a zone: {poly_e}. Calculating basic area."
                    )
                    area = zone.get('area', 0)
                    results['rooms'].append({
                        'points':
                ...
                st.warning(
                    f"Zone skipped due to insufficient points: {zone.get('id', 'N/A')}"
                )
...
def create_room_visualization(rooms):
    ...
    fig = go.Figure()
    colors = px.colors.qualitative.Plotly  # Use Plotly's qualitative colors for variety
    ...
    for i, room in enumerate(rooms):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hoverinfo='text',
                text=
                f"Room {i+1}<br>Area: {room['area']:.2f} m²<br>Layer: {room['layer']}<br>Type: {room.get('type', 'Unknown')}"
            ))
...
def display_direct_dwg_parser_interface():
    """Display the direct DWG parser interface"""
    st.header("Direct DWG File Parser")
    st.write(
        "This section allows for direct parsing and visualization of DWG/DXF files, independent of the main AI analysis workflow."
    )

    uploaded_file_direct = st.file_uploader(
        "Upload a DWG/DXF file for direct parsing",
        type=['dwg', 'dxf'],
        key="direct_parser_uploader")

    if uploaded_file_direct is not None:
        file_ext = uploaded_file_direct.name.lower().split('.')[-1]
        file_size_mb = uploaded_file_direct.size / (1024 * 1024)
        st.write(f"File: {uploaded_file_direct.name}")
        st.write(f"Size: {file_size_mb:.1f} MB")

        if st.button("Parse File Directly", key="parse_direct_button"):
            if file_size_mb > 50:  # Keep a stricter limit for direct parsing if desired
                st.error(
                    "File too large for direct parsing. Maximum size is 50MB.")
            else:
                with tempfile.NamedTemporaryFile(suffix=f'.{file_ext}',
                                                 delete=False) as tmp_file:
                    tmp_file.write(uploaded_file_direct.getvalue())
                    tmp_file_path = tmp_file.name

                try:
                    with st.spinner("Parsing DWG file..."):
                        results = parse_dwg_file_direct(tmp_file_path)
                    st.session_state.direct_parse_results = results
                    st.session_state.current_filename = uploaded_file_direct.name  # Update filename for display
                finally:
                    os.unlink(tmp_file_path)  # Clean up temp file

    # Display results if available
    if st.session_state.direct_parse_results:
        results = st.session_state.direct_parse_results
        if 'error' in results:
            st.error(f"Error parsing file: {results['error']}")
            st.session_state.direct_parse_results = None  # Clear results on error
        else:
            ...
            with col1:
                ...
                if results['rooms']:
                    for i, room in enumerate(results['rooms']):
                        ...
                        st.write(f"  - Type: {room.get('type', 'Unknown')}")
            ...
            with col1:
                ...
                st.download_button(
                    ...
                    data=export_data,
                    file_name=
                    f"{Path(st.session_state.current_filename).stem}_analysis.json",
                    mime="application/json")
            ...
            with col2:
                ...
                if results['rooms']:
                    ...
                    csv_buffer.write("Room,Area,Layer,Type,Source,Perimeter\n")

                    for i, room in enumerate(results['rooms']):
                        csv_buffer.write(
                            f"Room {i+1},{room['area']:.2f},{room['layer']},{room.get('type', 'Unknown')},{room.get('source', 'parsed')},{room.get('perimeter', 0):.2f}\n"
                        )
                    ...
                    st.download_button(
                        ...
                        data=csv_data,
                        file_name=
                        f"{Path(st.session_state.current_filename).stem}_rooms.csv",
                        mime="text/csv")
...
Room {i+1}:
  ...
  - Perimeter: {room.get('perimeter', 0):.2f} units
  - Points: {len(room['points'])} vertices
"""

                summary_text += f"""
Analysis Generated By: AI Architectural Space Analyzer
...
"""
                ...
                st.download_button(
                    ...
                    data=summary_text,
                    file_name=
                    f"{Path(st.session_state.current_filename).stem}_summary.txt",
                    mime="text/plain")

    st.divider()
    st.subheader(
        "Or specify file path directly (for local development/testing)")
    file_path_input = st.text_input("Enter full path to DWG/DXF file:",
                                    key="direct_path_input")

    if file_path_input and st.button("Parse from Path (Local)",
                                     key="parse_from_path_button"):
        if os.path.exists(file_path_input):
            try:
                results = parse_dwg_file_direct(file_path_input)
                st.session_state.direct_parse_results = results
                st.session_state.current_filename = Path(file_path_input).name
                st.rerun()  # Rerun to display results
            except Exception as e:
                st.error(f"Error parsing file from path: {str(e)}")
        else:
            st.error("File not found at the specified path.")


# --- End of Merged direct_dwg_parser.py functions ---
