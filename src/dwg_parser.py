...
class DWGParser:
    ...
    def parse_file(self, file_bytes: bytes, filename: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if file_extension == '.dwg':
                # Handle native DWG files
                zones = self._parse_dwg_file(temp_file_path)
            else:
                # Handle DXF files with ezdxf
                if EZDXF_AVAILABLE:
                    try:
                        ...
                        try:
                            doc, auditor = ezdxf.recover.readfile(temp_file_path)
                        except Exception:
                            # Create sample zones if both methods fail
                            return self._create_sample_zones()
                else:
                    return self._create_sample_zones()
    ...
    def _parse_dwg_file(self, file_path: str) -> List[Dict[str, Any]]:
        """Parse native DWG file using dxfgrabber"""
        zones = []
        
        if not DXFGRABBER_AVAILABLE:
            print("dxfgrabber not available, creating sample zones")
            return self._create_sample_zones()
        
        try:
            # Use dxfgrabber for DWG files
            drawing = dxfgrabber.readfile(file_path)
            
            # Extract entities from modelspace
            entities = list(drawing.modelspace())
            
            # Parse polylines and closed shapes
            for entity in entities:
                try:
                    if hasattr(entity, 'dxftype'):
                        ...
                        elif entity.dxftype == 'POLYLINE' and hasattr(entity, 'is_closed') and entity.is_closed:
                            if hasattr(entity, 'vertices'):
                                points = [(vertex.location[0], vertex.location[1]) for vertex in entity.vertices if hasattr(vertex, 'location')]
                                if len(points) >= 3:
                                    zones.append({
                                        'points': points,
                                        'layer': getattr(entity, 'layer', '0'),
            ...
            # If no zones found from actual geometry, create sample zones
            if not zones:
                print("No valid zones found in DWG file, creating sample zones")
                zones = self._create_sample_zones()
            
        except Exception as e:
            # Fallback: create sample zones if DWG parsing fails
            print(f"DWG parsing failed: {e}")
            zones = self._create_sample_zones()
        
        return zones
    
    def _approximate_arc_simple(self, arc_edge, num_segments: int = 8) -> List[tuple]:
        """Simple arc approximation for DWG files"""
        import math
        points = []
        ...
        try:
            ...
            start_angle = arc_edge.start_angle
            end_angle = arc_edge.end_angle
            ...
            # Return simple points if arc parsing fails
            points = [(0, 0), (1, 0), (1, 1), (0, 1)]
        
        return points
    
    def _create_sample_zones(self) -> List[Dict[str, Any]]:
        """Create sample zones when DWG parsing fails"""
        sample_zones = [
            {
