    ...
    if 'rooms' in results:
        ...
        with col2:
            ...
            if 'total_boxes' in results:
                box_area = results.get('total_boxes', 0) * results.get('parameters', {}).get('box_size', [2.0, 1.5])[0] * results.get('parameters', {}).get('box_size', [2.0, 1.5])[1]
        ...
        for zone in zones:
            if 'points' in zone and len(zone['points']) >= 3:
                try:
                    ...
                    st.warning(f"Shapely failed for a zone: {poly_e}. Calculating basic area.")
                    area = zone.get('area', 0)
                    results['rooms'].append({
                        'points': zone['points'],
                        'area': area,
                ...
                st.warning(f"Zone skipped due to insufficient points: {zone.get('id', 'N/A')}")
...
def create_room_visualization(rooms):
    ...
    for i, room in enumerate(rooms):
        ...
        fig.add_trace(go.Scatter(
            ...
            opacity=0.7,
            hoverinfo='text',
            text=f"Room {i+1}<br>Area: {room['area']:.2f} m²<br>Layer: {room['layer']}<br>Type: {room.get('type', 'Unknown')}"
        ))
...
def display_direct_dwg_parser_interface():
    ...
    st.write("This section allows for direct parsing and visualization of DWG/DXF files, independent of the main AI analysis workflow.")
    ...
    if uploaded_file_direct is not None:
        ...
        if st.button("Parse File Directly", key="parse_direct_button"):
            if file_size_mb > 50: # Keep a stricter limit for direct parsing if desired
                st.error("File too large for direct parsing. Maximum size is 50MB.")
            else:
                with tempfile.NamedTemporaryFile(suffix=f'.{file_ext}', delete=False) as tmp_file:
        ...
        if 'error' in results:
            ...
            with col1:
                ...
                st.download_button(
                    ...
                    data=export_data,
                    file_name=f"{Path(st.session_state.current_filename).stem}_analysis.json",
                    mime="application/json"
                )
            ...
            with col2:
                ...
                if results['rooms']:
                    ...
                    for i, room in enumerate(results['rooms']):
                        csv_buffer.write(f"Room {i+1},{room['area']:.2f},{room['layer']},{room.get('type', 'Unknown')},{room.get('source', 'parsed')},{room.get('perimeter', 0):.2f}\n")
                    ...
                    st.download_button(
                        ...
                        data=csv_data,
                        file_name=f"{Path(st.session_state.current_filename).stem}_rooms.csv",
                        mime="text/csv"
                    )
...
"""
                ...
                st.download_button(
                    ...
                    data=summary_text,
                    file_name=f"{Path(st.session_state.current_filename).stem}_summary.txt",
                    mime="text/plain"
                )

    st.divider()
    st.subheader("Or specify file path directly (for local development/testing)")
    file_path_input = st.text_input("Enter full path to DWG/DXF file:", key="direct_path_input")

    if file_path_input and st.button("Parse from Path (Local)", key="parse_from_path_button"):
